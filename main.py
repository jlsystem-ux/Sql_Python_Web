...
def exercises():
    ...
    # Convierte los datos en cadenas
    itemData = [(str(data[0]), str(data[1]), str(data[2]), str(data[3]), str(data[4]), str(data[5])) for data in itemData]
    return render_template("exercises.html", itemData=itemData, loggedIn=loggedIn, firstName=firstName, noOfItems=noOfItems, categoryData=categoryData)
